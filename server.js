...
io.on("connection", (socket) => {
  ...
  socket.on("join-as-viewer", (roomCode) => {
    ...
    if (room) {
      ...
      // Notify streamer about new viewer and updated count
      if (room.streamer) {
        io.to(room.streamer).emit("viewer-joined", socket.id);
        io.to(socket.roomCode).emit("viewer-count", room.viewers.size);
      }
    ...
    if (socket.roomCode) {
      ...
      if (room) {
        if (room.streamer === socket.id) {
          ...
          if (room.streamer) {
            io.to(room.streamer).emit("viewer-left", socket.id);
            io.to(socket.roomCode).emit("viewer-count", room.viewers.size);
          }
        }
      }
